...
public class ActivityFXReflect extends BaseActivity {
	...
	private Boolean isShowYL = false;

	private String alipayAccountId, WXAccountId, YLAccountId;
	...
	/** 银联账号 */
	@ViewInject(R.id.tv_yinlian_number)
	private TextView mYLnum;
	...
	@ViewInject(R.id.chk_yinlianpay)
	private RadioButton mChkYL;
	...
	@Event(value = { R.id.btn_return, R.id.btn_edit, R.id.tv_bind_account, R.id.chk_alipay, R.id.chk_yinlianpay,
			...
			if (ReflectCount == true) {
				if (isReflect == true) {
					...
					if (iii > 10) {
						...
						} else if (mChkYL.isChecked()) {
							typeid = "3";
							accountid = YLAccountId;
						}
				...
				if (jsonresult.equals("1")) {
					...
					for (int i = 0; i < jsonarray.length(); i++) {
						...
						if (mReflectBean.getUserAccountTypeId().equals("1")) {
							...
							isShowYL = true;
							mYLnum.setText(mReflectBean.getUserAccounts());
							mBankName.setText(mReflectBean.getUserAccountName());
							YLAccountId = mReflectBean.getUserAccountId();
						}
					...
					if (isShowYL == true) {
						mLLYL.setVisibility(View.VISIBLE);
					} else {
						mLLYL.setVisibility(View.GONE);
					}
